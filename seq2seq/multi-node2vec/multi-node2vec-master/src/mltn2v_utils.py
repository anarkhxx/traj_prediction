...
def parse_matrix_layers(network_dir, delim=',', binary=False, thresh=None):
    ...
    for network_file in os.listdir(network_dir):
        ...
        #print(file_path) It's all in the form of data/CONTROL_fmt\0040135.preprocess_v1.csv
        try:
            ...
            if thresh is not None:
                #print("aaa") thresh=0.5
                layer[layer <= thresh] = 0
            if binary:
                layer[layer != 0] = 1

            #print(layer),It is found that the data less than 0.5 in layer becomes 0, and the data greater than 0.5 becomes 1
            ...
            # In this way, the data of 0 will be eliminated and only the data of 1 will be left
            layer = layer.stack(dropna=True).reset_index()
            #print(layer)，Finally, only 3188 node pairs exist, which means 3188 edges (an example)
            # rename columns
